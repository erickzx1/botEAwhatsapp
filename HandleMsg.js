...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        pushname = pushname || verifiedName || formattedName // O nome verificado é o nome de alguém que usa uma conta comercial
        ...
        switch (command) {
                ...
                if (isOwnerBot) {
                    await aruga.joinGroupViaLink(linkgrup)
                        .then(async () => {
                            ...
                            await aruga.sendText(chekgrup.id, `Olá, Eu sou EA Bot. Para descobrir os comandos neste tipo de bot, escreva ${prefix}menu`)
                        })
                } else {
                ...
                rugaapi.ytmp3(`https://youtu.be/${linkmp3}`)
                    .then(async (res) => {
                        ...
                        await aruga.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
                            .catch(() => {
                                aruga.reply(from, `Este URL ${args[0]} Já foi baixado antes. O URL será redefinido após 1 hora / 60 minutos`, id)
                            })
                    })
                break
            case 'ytmp4':
            ...
            //Primbon Menu
            case 'cekzodiak':
                if (args.length !== 4) return aruga.reply(from, `Para verificar o seu signo do zodíaco, use ${prefix}nome ekzodíaco data de nascimento mês de nascimento ano de nascimento\nExemplo: ${prefix}cekzodiak ideia de 13 06 2004`, id)
            ...
            // Pesquisar qualquer
            case 'dewabatch':
                if (args.length == 0) return aruga.reply(from, `Para pesquisar anime em lote de Dewa Batch, tipo ${prefix}dewabatch judul\n\nExemplo: ${prefix}dewabatch naruto`, id)
                    ...
                    .then((result) => {
                        rugapoi.getVideo(result.link)
                            .then((res) => {
                                ...
                                aruga.reply(from, `Título: ${res.title}\n\nLink:\n${heheq}\nainda um testador :v`)
                            })
                    })
                    .catch(() => {
                        aruga.reply(from, 'Há um erro!', id)
            ...
            case 'lyrics':
            case 'lirik':
                if (args.length == 0) return aruga.reply(from, `Para pesquisar a letra de uma música\btipo: ${prefix}lirik [título da música]`, id)
                ...
                break
            case 'chord':
                if (args.length == 0) return aruga.reply(from, `Para pesquisar as letras e acordes de uma música\btipo: ${prefix}chord [título da música]`, id)
            ...
            case 'ss': //jika error silahkan buka file di folder settings/api.json dan ubah apiSS 'API-KEY' yang kalian dapat dari website https://apiflash.com/
                if (args.length == 0) return aruga.reply(from, `Faça a captura de tela dos bots em uma web\n\nUso: ${prefix}ss [url]\n\nexemplo: ${prefix}ss http://google.com`, id)
            ...
            case 'whatanime':
                if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                    ...
                    aruga.reply(from, 'Procurando....', id)
                    fetch('https://trace.moe/api/search', {
                        ...
                        .then(resolt => {
                            if (resolt.docs && resolt.docs.length <= 0) {
                                aruga.reply(from, 'Desculpe, não sei que anime é esse, certifique-se de que a imagem a ser pesquisada não está desfocada / cortada', id)
                            }
            ...
            //Dono do Grupo
            case 'kickall': //remover todos os membros
                if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos!', id)
            ...
            case 'bc': //para transmissão ou promoção
                ...
                break
            case 'leaveall': //removendo bots de todos os grupos e excluindo o chat
                ...
                break
            case 'clearall': //apaga todas as mensagens da conta do bot
                if (!isOwnerBot) return aruga.reply(from, 'Este comando é apenas para proprietários do Bot', id)
